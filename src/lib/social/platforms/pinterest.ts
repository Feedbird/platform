...
export class PinterestPlatform implements PlatformOperations {
  ...
  async refreshToken(acc: any): Promise<SocialAccount> {
    try {
      ...
      throw new Error(
        `Failed to refresh Pinterest token: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getToken(pageId: string): Promise<string> {
    try {
      ...
      if (!refreshedToken?.authToken) {
        throw new Error(
          'Failed to refresh Pinterest token. Please reconnect your Pinterest account.'
        );
      }
      ...
      await socialApiService.updateSocialPage(pageId, {
        ...
        authTokenExpiresAt: refreshedToken.accessTokenExpiresAt,
      });

      // update the social_accounts table with the new token
      await socialApiService.updateSocialAccount(pageData.accountId, {
        authToken: refreshedToken.authToken,
      ...
      throw new Error(
        'Failed to get Pinterest token. Please reconnect your Pinterest account.'
      );
    }
  }
    ...
    try {
      ...
      do {
        ...
        if (!response.items || !Array.isArray(response.items)) {
          console.error(
            'Invalid response structure from Pinterest API:',
            response
          );
  ...
  async publishPost(
    ...
    try {
      ...
      if (!options?.settings?.pinterest?.boardId) {
        throw new Error(
          'Pinterest requires a board to be selected. Please select a board in the settings.'
        );
      }
      ...
      if (mediaType === 'image' && mediaUrls.length === 1) {
        postedResponse = await this.publishImagePin(
          page,
          content,
          token,
          options
      ...
      if (mediaType === 'video' && mediaUrls.length === 1) {
        postedResponse = await this.publishVideoPin(
          page,
          content,
          token,
          options
      ...
      if (
        ...
        postedResponse = await this.publishMultiImagePin(
          page,
          content,
          token,
          options
  ...
  private async publishImagePin(
    ...
    const pin = await pinFetch<{ id: string }>(`${this.baseUrl}/pins`, {
      ...
      body: JSON.stringify({
        ...
        title:
          options?.settings?.pinterest?.title || 'Pin' + new Date().getTime(),
  ...
  private async publishVideoPin(
    ...
    if (uploadStatus !== 'succeeded') {
      throw new Error(
        `Video upload failed. Status: ${uploadStatus} after ${attempts} attempts`
      );
    }
    ...
    try {
      ...
      try {
        ...
        for (const board of boardsResponse.items) {
          try {
            ...
            console.warn(
              `[Pinterest] Failed to fetch pins from board ${board.id}:`,
              error
            );
          }
        }
      ...
      for (const boardId of boardIds) {
        try {
          ...
          console.warn(
            `[Pinterest] Failed to fetch board info for ${boardId}:`,
            error
          );
          // Set fallback info
          boardInfoMap.set(boardId, {
      ...
      const posts = response.items.map((pin) => {
        ...
        if (pin.media?.media_type === 'multiple_images' && pin.media?.items) {
          ...
          const url =
            ...
            (typeof imgs['1200x'] === 'object' &&
            imgs['1200x'] &&
            'url' in imgs['1200x']
              ? (imgs['1200x'] as any).url
              : undefined) ??
            (typeof imgs['600x'] === 'object' &&
            imgs['600x'] &&
            'url' in imgs['600x']
              ? (imgs['600x'] as any).url
              : undefined) ??
            (() => {
  ...
  async deletePost(page: SocialPage, postId: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Pinterest API error: ${response.status} ${response.statusText} - ${errorText}`
        );
      }
