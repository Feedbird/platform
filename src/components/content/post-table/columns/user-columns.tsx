...
import UserTextCell from "../user-text-cell";
import { SingleSelectCell } from "../single-select-cell";
import { MultiSelectCell } from "../multi-select-cell";
import { CheckboxCell } from "../checkbox-cell";
import { UserDateCell } from "../user-date-cell";
import { CreatedByCell, LastModifiedByCell } from "../user-cell";
import { MemoAttachmentCell } from "../memo-attachment-cell";
...
export function createUserColumns(params: {
  ...
  const cols: ColumnDef<Post>[] = userColumns.map((col) => {
    ...
    return {
      id: col.id,
      accessorFn: (row) => {
        if (col.type === "createdBy") return row.createdBy;
        if (col.id === "lastUpdatedBy") return row.lastUpdatedBy;
            ...
            return (
              <UserTextCell
                ...
                onValueCommit={(newVal) => {
                  ...
                  setTableData((prev) => prev.map((p) => (p.id === post.id ? { ...p, userColumns: newArr } : p)));
                  updatePost(post.id, { userColumns: newArr } as any);
                }}
              />
            );
          case "longText":
