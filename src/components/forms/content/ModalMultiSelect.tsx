...
export function ModalMultiSelect({
  ...
  const handleRemoveTag = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const newSelectedValues = selectedValues.filter((v) => v !== id);
    onSelectionChange(newSelectedValues);
  };
  ...
  return (
    <div ref={containerRef} className={cn("relative", className)}>
      ...
      <div
        ...
        onClick={() => !loadingParent && setIsOpen(!isOpen)}
      >
              ...
              {displayedTags.map((option) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={(e) => handleRemoveTag(option.id.toString(), e)}
                  >
                    <X className="h-3 w-3" />
                  </button>
                </div>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-1 z-50 rounded-md border border-gray-200 bg-white shadow-lg max-h-60 overflow-auto">
          {/* Search Input */}
              ...
              filteredOptions.map((option) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => handleToggleOption(option.id.toString())}
                  >
                    <span
