            ...
            <tr key={hg.id}>
              {hg.headers.map((header, index) => {
                ...
                return (
                  <th
                    key={header.id}
                    className={`px-3 py-2 ${isPostColumn ? 'text-left' : 'text-right'} text-xs font-medium text-darkGrey tracking-wider border-y ${!isLastColumn ? 'border-r border-strokeElement' : ''}`}
                  >
