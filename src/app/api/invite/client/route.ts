...
export async function POST(req: NextRequest) {
  try {
    ...
    // Force client role for Clerk and members table
    const memberRole: 'client' = 'client'
    const requestedRole: string = 'org:admin'

    // 1) Send organization invitation via Clerk
    ...
    try {
      ...
      // Board-only invites are not supported in client route

      // inviter name for template
        ...
        newRows.forEach(row => {
          if (row.is_workspace) {
            const workspaceActivity = supabase
              .from('activities')
              .insert({ workspace_id: row.workspace_id, post_id: null, type: 'workspace_invited_sent', actor_id: actorId, metadata: { invitedEmail: email, workspaceId: row.workspace_id } })
    ...
    if (clerkInvitationSent) {
      return NextResponse.json({ message: 'Organization invitation sent successfully', details: `Invited to ${newRows.length} workspace(s)/board(s)` })
    } else {
