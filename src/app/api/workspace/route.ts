  ...
  try {
    ...
    if (id) {
      ...
      const { data: ws, error } = await supabase
        .from('workspaces')
        .select(
          'id, name, logo, created_by, default_board_rules, timezone, week_start, time_format, allowed_posting_time'
        )
        .eq('id', id)
        .single();
      ...
      if (accountsError) {
        console.error(
          'Error fetching workspace social accounts:',
          accountsError
        );
        return NextResponse.json(
      ...
      const invitedWorkspaceIds = Array.from(
      ...
      const buildWs = (ws: any, role: 'admin' | 'member') => {
        ...
        if (role === 'admin') {
          ...
          if (hasWorkspaceInvite) {
            ...
            const allowedBoardIds = rowsForWs
              .filter((r) => !r.is_workspace)
              .map((r) => r.board_id);
        ...
        return {
          ...
          role,
          boards: wsBoards,
          social_accounts: socialsByWorkspace[ws.id] || [],
          social_sets: setsByWorkspace[ws.id] || [],
        };
      };
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.error(
        'Failed to create Clerk organization for workspace:',
        orgErr
      );
      return NextResponse.json(
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    try {
      const organizationId = (workspace as any).clerk_organization_id as
        | string
        | null
        | undefined;
      if (organizationId) {
        ...
        try {
          ...
          console.log(
            `Deleted Clerk organization ${organizationId} for workspace ${id}`
          );
          ...
          if (isNotFound) {
            console.warn(
              `Clerk organization ${organizationId} not found; continuing workspace deletion`
            );
          } else {
      ...
      console.error(
        'Unexpected error during Clerk organization cleanup step:',
        orgPrecheckErr
      );
      return NextResponse.json(
    ...
    try {
      ...
      if (socialPagesByWorkspaceError) {
        console.error(
          'Error deleting social pages by workspace:',
          socialPagesByWorkspaceError
        );
      ...
      if (socialAccountsByWorkspaceError) {
        console.error(
          'Error deleting social accounts by workspace:',
          socialAccountsByWorkspaceError
        );
      ...
      return NextResponse.json(
        {
          ...
          suggestion:
            'Please ensure all posts, boards, and other related data are removed before deleting the workspace',
        },
