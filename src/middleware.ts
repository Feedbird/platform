...
async function limitRequest(
  ...
  if (entry.count < limit) {
    ...
    return {
      ...
      reset: entry.windowStart + windowSeconds * 1000,
    };
  }
  return {
    success: false,
...
export default clerkMiddleware(async (auth, req) => {
  ...
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') {
    const maxBytes = parseSizeToBytes(
      process.env.API_MAX_BODY || process.env.NEXT_PUBLIC_API_MAX_BODY || '2mb'
    );
